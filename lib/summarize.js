...
async function summarize (story, shortenSentences, numSentences) {
    ...
    if (shortenSentences) {
        var next_sentences = await clean_sentences(sentences);
        ...
        for(var i = 0; i < sentences.length; i += 1) {
            ...
            // Only allow for "short" sentences. Ideally less than 24 words, but
            // technical documentation may be longer.
            if (words.length <= 30) {
                new_sentences.push(sentence);
            }
        }
        if (new_sentences.length >= numSentences) {
...
async function clean_sentences(sentences) {
    ...
    sentences.forEach((sen, sen_id) => {
        var words = tokenizer.tokenize(sen);
